...
<div class="py-6 mx-auto max-w-7xl sm:px-6 lg:px-8">
    <div class="px-4 py-6 sm:px-0">
        ...
        <div class="flex items-center justify-between mb-8">
            <div class="flex items-center space-x-4">
                ...
                <div class="flex-shrink-0">
                    ...
                    @else
                        <div class="flex items-center justify-center w-16 h-16 bg-gradient-to-br from-blue-500 to-blue-600 rounded-full border-4 border-white shadow-lg">
                            <span class="text-xl font-bold text-white">{{ substr($teacher->user->name, 0, 1) }}</span>
                        </div>
                    @endif
                </div>
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Edit Teacher Profile</h1>
                    <p class="mt-2 text-sm text-gray-600">Update {{ $teacher->user->name }}'s information and assignments</p>
                </div>
            </div>
        ...
        <!-- Success Message -->
        @if (session('success'))
            <div class="p-4 mb-6 text-green-700 bg-green-50 border border-green-200 rounded-xl" role="alert">
            ...
            <div class="p-8 bg-white border-b border-gray-200">
                <form method="POST" action="{{ route('admin.teachers.update', $teacher) }}" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <div class="flex items-center mt-1">
                                    <div class="flex-shrink-0">
                                        @if($teacher->profile_image)
                                            <img src="{{ $teacher->profile_image_url }}" alt="Current Profile Image" 
                                                class="w-16 h-16 rounded-full object-cover border-2 border-gray-300">
                                        @else
                                            <div class="flex items-center justify-center w-16 h-16 bg-gray-200 rounded-full border-2 border-gray-300">
                            ...
                            <div>
                                ...
                                <input type="number" name="years_of_experience" id="years_of_experience" value="{{ old('years_of_experience', $teacher->years_of_experience) }}" min="0" required
                                    class="block w-full px-3 py-2 mt-1 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            ...
                            <div>
                                ...
                                <input type="date" name="hire_date" id="hire_date" value="{{ old('hire_date', $teacher->hire_date->format('Y-m-d')) }}" required
                                    class="block w-full px-3 py-2 mt-1 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            ...
                            <div>
                                ...
                                <input type="text" name="office_location" id="office_location" value="{{ old('office_location', $teacher->office_location) }}"
                                    class="block w-full px-3 py-2 mt-1 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            ...
                            <div>
                                ...
                                <input type="text" name="office_hours" id="office_hours" value="{{ old('office_hours', $teacher->office_hours) }}" placeholder="e.g., Mon-Wed 2-4 PM"
                                    class="block w-full px-3 py-2 mt-1 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <textarea name="bio" id="bio" rows="4" placeholder="Tell us about the teacher's background, expertise, and interests..."
                                    class="block w-full px-3 py-2 mt-1 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">{{ old('bio', $teacher->bio) }}</textarea>
                    ...
                    <div class="mb-10">
                        ...
                        <div class="mb-8">
                            <label class="block mb-4 text-lg font-medium text-gray-900">Department Assignment *</label>
                            <div class="p-6 bg-gray-50 rounded-xl">
                                <p class="mb-4 text-sm text-gray-600">Select departments this teacher belongs to and choose one as primary department.</p>
                                
                                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                    @foreach($departments as $department)
                                    @php
                                        $isAssigned = $teacher->departments->contains($department->id);
                                        $isPrimary = $isAssigned && $teacher->departments->find($department->id)->pivot->is_primary;
                                    @endphp
                                    <div class="relative flex items-start p-4 bg-white border border-gray-200 rounded-lg hover:border-blue-300 transition-colors {{ $isAssigned ? 'border-blue-200 bg-blue-50' : '' }}">
                                        ...
                                        <div class="ml-3 text-sm">
                                            <label for="department_{{ $department->id }}" class="font-medium text-gray-700">
                                                {{ $department->name }}
                                            </label>
                                            ...
                                            <div class="mt-2">
                                                ...
                                                <label for="primary_{{ $department->id }}" class="ml-1 text-xs font-medium text-blue-600">
                                                    Set as Primary
                                                </label>
                                            </div>
                                        </div>
                                ...
                                @error('primary_department') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>
                        </div>
                        ...
                        <div class="mb-8">
                            ...
                            <div class="p-6 bg-gray-50 rounded-xl">
                                ...
                                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                                    ...
                                    @php
                                        $isAssigned = $teacher->subjects->contains($subject->id);
                                    @endphp
                                    <div class="relative flex items-start p-3 bg-white border border-gray-200 rounded-lg hover:border-green-300 transition-colors {{ $isAssigned ? 'border-green-200 bg-green-50' : '' }}">
                                        <div class="flex items-center h-5">
                        ...
                        <div>
                            ...
                            <div class="p-6 bg-gray-50 rounded-xl">
                                ...
                                <div class="grid grid-cols-1 gap-3">
                                    ...
                                    @php
                                        $isAssigned = $teacher->classGroups->contains($classGroup->id);
                                    @endphp
                                    <div class="relative flex items-start p-3 bg-white border border-gray-200 rounded-lg hover:border-orange-300 transition-colors {{ $isAssigned ? 'border-orange-200 bg-orange-50' : '' }}">
                                        <div class="flex items-center h-5">
                    ...
                    <div class="flex items-center justify-between pt-6 mt-8 border-t border-gray-200">
                        ...
                        <div class="flex space-x-3">
                            <a href="{{ route('admin.teachers.manage-assignments', $teacher) }}" 
                               class="inline-flex items-center px-6 py-3 text-sm font-medium text-green-600 bg-green-50 border border-green-200 rounded-lg shadow-sm hover:bg-green-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentAssignments = document.querySelectorAll('.bg-blue-50, .bg-green-50, .bg-orange-50');
